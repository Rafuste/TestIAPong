...
# This loop will run if the game is active and 'Reproducción Automática' is checked.
if st.session_state.running and not st.session_session.game.game_over:
    st.session_state.game.update()
    # Auto Play functionality: automatically rerun the script to simulate continuous frames
    auto_play = st.sidebar.checkbox("Reproducción Automática", value=False)
    if auto_play and not st.session_state.game.game_over:
...
st.info("Nota: Para mover el paddle del jugador, utiliza el deslizador 'Posición Vertical del Paddle' en la barra lateral. Para un juego más fluido, activa la casilla 'Reproducción Automática' en la barra lateral.")
st.markdown("---")
